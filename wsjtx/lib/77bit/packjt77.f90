...
logical function check_grid4(grid4)
    implicit none
    
    character*4 , intent(in) :: grid4
    
    character*4 grid4_tr
    
    grid4_tr = trim(grid4)
  
    check_grid4 = len(grid4_tr).eq.4 .and.                        &
...
end function check_grid4 

logical function check_grid6(grid6)
    implicit none
    
    character*6 , intent(in) :: grid6
    character*6 grid6_tr
    integer result
    
    grid6_tr = trim(grid6)
  
    check_grid6 = (len((grid6_tr)).eq.6.or.len((grid6_tr)).eq.4).and. &
...
! WSPR Type 1
     if(.not.check_grid4(w(2)(1:4))) go to 900
...
! WSPR Type 3
     ...
     grid6=w(2)(1:6)
     if(.not.check_grid6(grid6)) go to 900
     i3=0
     n3=6
     call pack28(w(1),n28)
     n22=n28-2063592
...
! Example message:  WA9XYZ/R KA1ABC/R R FN42     28 1 28 1 1 15   74
  ...
  c1=w(nwords)(1:1)
  c2=w(nwords)(1:2)
  if(.not.check_grid4(w(nwords)(1:4)) .and. c1.ne.'+' .and. c1.ne.'-'              &
...
2 if(nx.lt.520001 .or. nx.gt.594095) return   !Exchange between 520001 - 594095
  if(.not.check_grid6(w(nwords)(1:6))) return    !Last word must be a valid grid6
